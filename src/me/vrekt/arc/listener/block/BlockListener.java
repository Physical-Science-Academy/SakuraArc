...
public final class BlockListener implements Listener {
    ...
    private void onBlockBreak(BlockBreakEvent event) {
        ...
        if (!nuker.isPacketCheck()) {
            SakuraAPIAB api = new SakuraAPI();
            api.addVL(player, "BadPacket", 2.5d, "Arc: Failed Nuker");
            event.setCancelled(nuker.check(player));
        }
        boolean fastBreakResult = fastBreak.check(player, BlockData.get(player), event.getBlock());
        if (fastBreakResult) {
            SakuraAPIAB api = new SakuraAPI();
            api.addVL(player, "BadPacket", 1d, "Arc: Failed FastBreaker");
            event.setCancelled(fastBreakResult);
        }
    }
